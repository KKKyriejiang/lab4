...
typedef struct {
    int k;       // 当前消去的行
    int t_id;    // 线程ID
    char padding[CACHE_LINE_SIZE - 2*sizeof(int)]; // 避免伪共享
} threadParam_t;

// 缓存优化: 初始化矩阵，使用连续内存存储
void init_matrix() {
    // 分配连续内存块
    matrix_data = new float[n * n];
    matrix = new float*[n];
...
// 缓存优化: NEON向量化的行归一化函数 (按块处理)
void division_neon_block(int k, int t_id, int cols_per_thread) {
    // 计算线程负责的列范围
    ...
    // 缓存优化: 使用分块处理提高缓存命中率
    const int BLOCK_SIZE = 16; // 块大小，根据L1缓存大小调整
    
    for (int j_block = col_start; j_block < col_end; j_block += BLOCK_SIZE) {
        int j_limit = min(j_block + BLOCK_SIZE, col_end);
...
// 缓存优化: NEON向量化的消去函数 (按块处理)
void elimination_neon_block(int k, int i, int t_id, int cols_per_thread) {
    // 计算线程负责的列范围
    int col_start = k + 1 + t_id * cols_per_thread;
    int col_end = col_start + cols_per_thread;
    if (col_end > n) col_end = n;
...
void gaussEliminationSerial() {
    ...
    for (int k = 0; k < n; k++) {
        ...
        for (int j_block = k + 1; j_block < n; j_block += BLOCK_SIZE) {
            int j_limit = min(j_block + BLOCK_SIZE, n);
        ...
        for (int i_block = k + 1; i_block < n; i_block += BLOCK_SIZE) {
            int i_limit = min(i_block + BLOCK_SIZE, n);
...
void* dynamicDivisionThreadFunc(void* param) {
    ...
    // 执行块划分的归一化
    division_neon_block(k, t_id, cols_per_thread);
    
    pthread_exit(NULL);
}

// 1. 动态线程版本的消去线程函数 (缓存优化)
void* dynamicEliminationThreadFunc(void* param) {
    ...
    // 执行块划分的消去
    elimination_neon_block(k, i, t_id, cols_per_thread);
    
    pthread_exit(NULL);
}

// 1. 缓存优化: 动态线程版本的高斯消去算法（列划分 + 分块）
void gaussEliminationDynamicThread() {
    for (int k = 0; k < n; k++) {
        ...
        // 缓存优化: 使用任务队列方式减少线程创建开销，每个线程处理多行
        const int ROWS_PER_TASK = 4;  // 每个任务处理的行数
        int task_count = (n - k - 1 + ROWS_PER_TASK - 1) / ROWS_PER_TASK * NUM_THREADS;
        
        // 为每个线程分配任务（行和列的二维划分）
        int thread_idx = 0;
        for (int i_block = k + 1; i_block < n; i_block += ROWS_PER_TASK) {
            int i_limit = min(i_block + ROWS_PER_TASK, n);
            for (int i = i_block; i < i_limit; i++) {
                for (int t_id = 0; t_id < NUM_THREADS; t_id++) {
                    elimination_param[thread_idx].k = k;
                    elimination_param[thread_idx].t_id = (i - k - 1) * NUM_THREADS + t_id;
                    pthread_create(&elimination_handles[thread_idx], NULL, dynamicEliminationThreadFunc, &elimination_param[thread_idx]);
                    thread_idx++;
                    if (thread_idx >= elimination_thread_count) break;
                }
                if (thread_idx >= elimination_thread_count) break;
            }
            if (thread_idx >= elimination_thread_count) break;
        ...
        // 等待所有消去线程完成
        for (int t_id = 0; t_id < thread_idx; t_id++) {
...
// 2. 静态线程+信号量同步版本的线程函数（缓存优化）
void* staticSemaphoreThreadFunc(void* param) {
    threadParam_t* p = (threadParam_t*)param;
    int t_id = p->t_id;
    
    // 缓存优化: 预取部分数据到L1缓存
    const int PREFETCH_BLOCK = 16;
    ...
    for (int k = 0; k < n; k++) {
        ...
        // 缓存优化: 按块处理多行，提高数据局部性
        const int ROW_BLOCK_SIZE = 4;  // 处理的行块大小
        
        // 进行消去操作（每个线程处理所有行的对应列部分）
        for (int i_block = k + 1; i_block < n; i_block += ROW_BLOCK_SIZE) {
            int i_limit = min(i_block + ROW_BLOCK_SIZE, n);
...
// 3. 静态线程+信号量同步+三重循环全部纳入线程函数版本的线程函数（缓存优化）
void* staticFullThreadFunc(void* param) {
    threadParam_t* p = (threadParam_t*)param;
    int t_id = p->t_id;
    
    // 缓存优化: 预先计算线程的工作范围
    int cols_per_thread;  // 动态计算
    
    for (int k = 0; k < n; k++) {
        // 计算每个线程需要处理的列数
        cols_per_thread = (n - k - 1) / NUM_THREADS + 1;
...
// 3. 静态线程+信号量同步+三重循环全部纳入线程函数版本的高斯消去算法（缓存优化）
...
// 4. 静态线程+barrier同步版本的高斯消去算法（缓存优化）
