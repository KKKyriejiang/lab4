...
// AVX向量化的行归一化函数（按列划分）
void division_avx_col(int k, int t_id, int cols_per_thread) {
    // 计算线程负责的列范围
...
// AVX向量化的消去函数（按列划分）
void elimination_avx_col(int k, int i, int t_id, int cols_per_thread) {
    // 计算线程负责的列范围
    int col_start = k + 1 + t_id * cols_per_thread;
    int col_end = col_start + cols_per_thread;
    if (col_end > n) col_end = n;
    ...
    __m256 vaik = _mm256_set1_ps(multiplier);
    int j = col_start;
    
    // 使用AVX指令进行向量化消去，每次处理8个浮点数
    for (; j + 7 < col_end; j += 8) {
...
void* dynamicDivisionThreadFunc(void* param) {
    ...
    // 执行列划分的归一化
    division_avx_col(k, t_id, cols_per_thread);
...
void* dynamicEliminationThreadFunc(void* param) {
    ...
    // 执行列划分的消去
    elimination_avx_col(k, i, t_id, cols_per_thread);
...
int main(int argc, char** argv) {
    cout << "Gaussian Elimination with AVX (Column Division) Implementation" << endl;
    cout << "Current Date and Time (UTC): 2025-05-22 01:13:56" << endl;
